...
# Set to 15 to match the chart layout in the fact sheet screenshot.
TOP_N = 15
...
# ============================================================
# FACTAARK PREVIEW FORMATTING AND CHART IMAGES
# ============================================================
...
def make_chart_png(table: pd.DataFrame, label_col: str, title: str, output_png: Path) -> None:
    """Create a chart image that matches the fact-sheet visual style."""
    try:
        import matplotlib.pyplot as plt
        from matplotlib.ticker import MultipleLocator
    except ImportError:
        sys.exit("matplotlib is missing. Install it with: pip install matplotlib pillow")

    plot_table = table.copy().reset_index(drop=True)
    if plot_table.empty:
        return

    labels = plot_table[label_col].astype(str).tolist()
    values = pd.to_numeric(plot_table["Mio. GT"], errors="coerce").fillna(0).tolist()
    countries = plot_table["Country"].astype(str).tolist()
    colors = [DENMARK_COLOR if normalize_country_name(c).casefold() == "denmark" else BAR_COLOR for c in countries]

    n = len(plot_table)
    y_positions = list(range(n))
    max_value = max(values) if values else 1
    x_limit = max_value * 1.22

    fig, ax = plt.subplots(figsize=(7.4, 4.15), dpi=220)
    fig.patch.set_facecolor("white")
    ax.set_facecolor("white")

    ax.barh(y_positions, values, color=colors, height=0.35)
    ax.set_yticks(y_positions)
    ax.set_yticklabels(labels, fontsize=7.3, color=TITLE_COLOR)
    ax.invert_yaxis()

    ax.set_title(title, fontsize=11, fontweight="bold", color=TITLE_COLOR, pad=12)
    ...
    # Hide x-axis numbers and ticks. Keep vertical gridlines.
    ax.tick_params(axis="x", labelbottom=False, length=0)
    ax.tick_params(axis="y", length=0)

    # Minimal frame, similar to Excel chart area.
    for spine in ["top", "right", "bottom"]:
        ax.spines[spine].set_visible(False)
    ax.spines["left"].set_color("#BFBFBF")
    ax.spines["left"].set_linewidth(0.6)

    # Data labels outside bar ends, with Danish comma decimal.
    for y, value in zip(y_positions, values):
        label = f"{value:.2f}".replace(".", ",")
        ax.text(value + max_value * 0.015, y, label, va="center", ha="left", fontsize=7.2, color=TITLE_COLOR)

    fig.subplots_adjust(left=0.25, right=0.97, top=0.86, bottom=0.08)
    fig.savefig(output_png, dpi=220, facecolor="white")
    plt.close(fig)


def create_top20_chart_sheet(preview_path: Path, df: pd.DataFrame) -> None:
    """Create Top 20 sheet with chart images. This avoids blank native Excel charts."""
    ...
    # Store source tables below the images so the numbers are auditable.
    ...
    # Write source data under the charts.
    start_row = 26
    ws.cell(row=start_row, column=2, value="Opererede flåder - grafdata").font = Font(bold=True)
    for j, col_name in enumerate(operator_table.columns, start=2):
        ws.cell(row=start_row + 1, column=j, value=col_name).font = Font(bold=True)
    for i, (_, row) in enumerate(operator_table.iterrows(), start=start_row + 2):
        for j, col_name in enumerate(operator_table.columns, start=2):
            cell = ws.cell(row=i, column=j, value=row[col_name])
            if col_name == "Mio. GT":
                cell.number_format = "0.00"

    ws.cell(row=start_row, column=6, value="Flagstater - grafdata").font = Font(bold=True)
    for j, col_name in enumerate(flag_table.columns, start=6):
        ws.cell(row=start_row + 1, column=j, value=col_name).font = Font(bold=True)
    for i, (_, row) in enumerate(flag_table.iterrows(), start=start_row + 2):
        for j, col_name in enumerate(flag_table.columns, start=6):
            cell = ws.cell(row=i, column=j, value=row[col_name])
            if col_name == "Mio. GT":
                cell.number_format = "0.00"
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        operator_png = tmp / "operator_chart.png"
        flag_png = tmp / "flag_chart.png"

        make_chart_png(operator_table, "Operatørland", "Største opererede flåder i mio GT.", operator_png)
        make_chart_png(flag_table, "Flag", "Største flagstater i mio GT.", flag_png)

        if operator_png.exists():
            img1 = XLImage(str(operator_png))
            img1.width = 690
            img1.height = 380
            ws.add_image(img1, "B2")

        if flag_png.exists():
            img2 = XLImage(str(flag_png))
            img2.width = 690
            img2.height = 380
